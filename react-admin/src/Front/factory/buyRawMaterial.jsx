    ...
    const ConfirmShow = (props) =>{
      ...
      const  BuyCotton = () =>{
        ...
        const buyCottonMaterialHandler = async (event) => {
          // console.log("Cotton",event.cottonWeight);
          const tx = supplyChainContract.factoryBuyRawMaterial(data.state.id, [event.cottonWeight, event.fiberlength, event.fiberStrength, event.cottonMike, event.fqi]);
        ...
        const [fqi, setFqi] = useState(data.state.rawMaterial1);
      ...
      const  BuyPolyester = () =>{
        ...
        const buyPolyesterMaterialHandler = async (event) => {
          const tx = supplyChainContract.factoryBuyRawMaterial(data.state.id, [event.polyesterFmax, event.polyesteremax, event.polyesterNeps, event.polysterCvm, 0]);
          //console.log((await tx.wait()));
          if(tx){
             navigate("/availableRawMaterialToBuy")
          }
        ...
        const handleSubmit = (e) =>{
          ...
          buyPolyesterMaterialHandler(data)
        }
      ...
      const  BuyWool = () =>{
        ...
        const buyWoolMaterialHandler = async (event) => {
          const tx = supplyChainContract.factoryBuyRawMaterial(data.state.id, [event.woolDialmeter, event.woolStapleLength, event.woolFiberLength, event.woolCrimpiness, 0]);
          //console.log((await tx.wait()));
          if(tx){
             navigate("/availableRawMaterialToBuy")
          }
